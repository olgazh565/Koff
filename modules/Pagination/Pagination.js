export class Pagination {
	...
	update({
		...
		this.paginationBar.style.setProperty(
				'--width',
				`calc(${width < totalProducts ? width : totalProducts
				} / ${totalProducts} * 100%)`,
		);
		this.paginationCurrent.textContent =
			totalProducts === limit ? totalProducts :
	...
	createPagination() {
		...
		this.paginationLeft.classList.add(
				'pagination__arrow',
				'pagination__arrow_left',
		);
		this.paginationLeft.ariaLabel = 'Пагинация назад';
    ...
    `;
		...
		this.paginationRight.classList.add(
				'pagination__arrow',
				'pagination__arrow_right',
		);
		this.paginationRight.ariaLabel = 'Пагинация вперед';
