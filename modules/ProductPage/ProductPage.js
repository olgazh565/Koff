...
export class ProductPage {
	...
	render(data) {
		...
		productMainList.append(...mainSlederItems);
      ...
      `;
			...
			const thumbnailItems = data.images.map(item => {
				const thumbnailSlide = document.createElement('div');
				thumbnailSlide.classList.add(
						'swiper-slide',
						'product__thumbnails-slide',
				);

				const thumbnailImg = document.createElement('img');
				thumbnailImg.classList.add('product__thumbnails-img');
				thumbnailImg.src = `${API_URL}${item}`;

				thumbnailSlide.append(thumbnailImg);

				return thumbnailSlide;
			});

			productThumbnailsList.append(...thumbnailItems);
		...
		const productChararteristics = document.createElement('div');
		productChararteristics.classList.add('product__characteristics');
		...
		const chararteristicsList = document.createElement('ul');
		chararteristicsList.classList.add('product__characteristics-list');

		const characteristicsItems = data.characteristics.map(item => {
			const characteristicsItem = document.createElement('li');
			characteristicsItem.classList.add('product__characteristics-item');

			const characteristicsName = document.createElement('p');
			characteristicsName.classList.add('product__characteristics-name');
			characteristicsName.textContent = item[0];

			const characteristicsValue = document.createElement('p');
			characteristicsValue.classList.add('product__characteristics-value');
			characteristicsValue.textContent = item[1];

			characteristicsItem.append(characteristicsName, characteristicsValue);
		...
		const productBtn = new CardButton('product__btn', 'В корзину')
				.create(data.id);
		const productLikeButton = new LikeButton('product__favorite')
				.create(data.id);

		productButtons.append(productBtn, productLikeButton);

		chararteristicsList.append(...characteristicsItems);
		productChararteristics.append(
				characteristicsTitle,
				chararteristicsList,
				productButtons,
		);
		productInfo.append(productPrice, productArticle, productChararteristics);
